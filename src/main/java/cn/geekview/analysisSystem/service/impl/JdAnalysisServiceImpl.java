...
import cn.geekview.analysisSystem.entity.dao.TdreamJdAnalysisMapper;
import cn.geekview.analysisSystem.entity.dao.TdreamJdProjectMapper;
...
import cn.geekview.analysisSystem.remotedata.entity.dao.JdItemMapper;
import cn.geekview.analysisSystem.remotedata.entity.dao.JdProjectMapper;
...
@Service("JdAnalysisServiceImpl")
public class JdAnalysisServiceImpl implements IAnalysisService {
	
	private SimpleDateFormat dateFormat = new SimpleDateFormat("yyyy-MM-dd hh:mm:ss");
	
	@Autowired
	private JdProjectMapper jdProjectMapper;
	
	@Autowired
	private JdItemMapper jdItemMapper;
	
	@Autowired
	private TdreamJdProjectMapper tdreamJdProjectMapper;
	...
	@Autowired
	private TdreamJdAnalysisMapper tdreamJdAnalysisMapper;
	
	/**
	 * 如果在分析的过程中，出现异常，需要重新分析，会不会对已经分析的数据造成影响？
	 * 		事务回滚
	 * 		解决办法：删除当日的平台分析表（t_dream_jd_analysis）和增速表（t_dream_rank_growth）的数据
	 * 	
	...
	public int doWebsiteAnalysis(Date updateDate,Logger logger) {
		logger.info("京东平台开始分析");
		//1、平台需要被分析的的产品
		long start = System.currentTimeMillis();
		List<TdreamJdProject> listByUpdateDate = jdProjectMapper.selectListByUpdateDate(updateDate);
		long end = System.currentTimeMillis();
		logger.info("查询平台需要被分析的的产品花费"+(end-start)/1000+"秒");
		Map<String,Object> parmMap = new HashMap<String, Object>();
		Integer productId = null;
		for (TdreamJdProject tdreamJdProject : listByUpdateDate) {
			String originalId = tdreamJdProject.getOriginalId();
			//2、据平台编号和原始id查询t_dream_product表中
			TdreamProduct tdreamProduct = tdreamProductMapper.selectByOriginalId(2, originalId);
			if(tdreamProduct==null){
				tdreamJdProject.setBeginDate(new Date());
				//不存在，插入的产品到t_dream_product表
				tdreamJdProjectMapper.insertProduct(tdreamJdProject);
				productId = tdreamProductMapper.selectProductId(2,originalId);//改
			}else{
				productId = tdreamProduct.getPkId();
				tdreamJdProject.setBeginDate(tdreamProduct.getProductBegin());
				//存在，更新的产品到t_dream_product表
				tdreamJdProjectMapper.updateProduct(tdreamJdProject);
			}
			/**
			 * 处理当天重新分析，以及多次分析的情况
			 * 	根据更新时间删除当日的平台分析表（t_dream_jd_analysis）和增速表（t_dream_rank_growth）的数据
			 */
			tdreamJdAnalysisMapper.deleteByProductId(productId,updateDate);
			 ...
			 * 先处理已经分类的产品，再进行分词分类后
			 */
			TdreamJdAnalysis entity = creatPrepareEntity(tdreamJdProject);
			entity.setProductId(productId);
			
			tdreamJdAnalysisMapper.insert(entity);
			tdreamJdAnalysisMapper.updateProduct(entity);
			
			if(tdreamProduct!=null&&tdreamProduct.getProductEnabled()==1){
				TdreamJdAnalysis prevEntity = tdreamJdAnalysisMapper.
						selectPrevAnalysis(productId,tdreamJdProject.getUpdateDate());//传入产品抓取回来传入数据库的时间：否则t_dream_rank_growth表的上一次金额为空
				insertRankGrowth(tdreamJdProject,entity,prevEntity,logger);
			}
		}
		long end2 = System.currentTimeMillis();
		logger.info("京东平台分析花费"+(end2-start)/1000+"秒");
		logger.info("京东平台分析结束");
		return 1;
	}
	
	
	
	private void insertRankGrowth(TdreamJdProject project, TdreamJdAnalysis entity, TdreamJdAnalysis prevEntity, Logger logger) {
		int days = 1;
	...
	private  TdreamJdAnalysis creatPrepareEntity(TdreamJdProject project){
		TdreamJdAnalysis entity = new TdreamJdAnalysis();
		...
		if(project.getItemList()!=null&&project.getItemList().size()>0){
			List<TdreamJdItem> limitList = new ArrayList<TdreamJdItem>();//支持人数无限额的支持档
			List<TdreamJdItem> unLimitList = new ArrayList<TdreamJdItem>();//支持人数有限额的支持档
			for(TdreamJdItem item : project.getItemList()){
			...
			if(limitList.size()>0){
				for(TdreamJdItem item : limitList){
				...
				if(unLimitList.size()>0){
					for(TdreamJdItem item : unLimitList){
						if(item.getItemPrice().intValue()==1){//如果支持档的价格是1，跳过执行下一次循环
							continue;
						}
						if(itemMap.get(item.getItemPrice())!=null){
	...
	public int doProductAnalysis(Integer originId) {
		List<TdreamJdProject> projectList = null; //产品列表
		List<TdreamJdItem> itemsList = null;	  //产品项列表
		...
		//获取指定的产品列表
		projectList = jdProjectMapper.getProductByOriginIdWithDaySpace(originId);
		...
		for (TdreamJdProject tdreamJdProject : projectList) {
			//获取要展示产品的数据
			productName = tdreamJdProject.getProjectName();
			supportPersonList.add(String.valueOf(tdreamJdProject.getSupportPerson()));
			finishPercentList.add(String.valueOf(tdreamJdProject.getFinishPer()));
			currentMoneyList.add(String.valueOf(tdreamJdProject.getCurrMoney()));
			updateDateList.add(dateFormat.format(tdreamJdProject.getUpdateDate()));
			//获取这个产品下的项目
			itemsList = jdItemMapper.getItemsByProjectId(tdreamJdProject.getPkId());
			if(itemsList==null||itemsList.size()==0){
				continue;
			}
			itemnum = itemsList.size();
			for (TdreamJdItem tdreamJdIterm : itemsList) {
				
				BigDecimal 	price = (BigDecimal) tdreamJdIterm.getItemPrice();
				Integer target = (Integer) tdreamJdIterm.getItemTotal();
				Integer actual = (Integer) tdreamJdIterm.getItemSupport();
