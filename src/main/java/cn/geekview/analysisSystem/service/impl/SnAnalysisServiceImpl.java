...
import cn.geekview.analysisSystem.entity.dao.TdreamSnAnalysisMapper;
import cn.geekview.analysisSystem.entity.dao.TdreamSnProjectMapper;
...
import cn.geekview.analysisSystem.entity.model.TdreamSnItem;
...
import cn.geekview.analysisSystem.remotedata.entity.dao.SnItemMapper;
import cn.geekview.analysisSystem.remotedata.entity.dao.SnProjectMapper;
...
@Service("SnAnalysisServiceImpl")
public class SnAnalysisServiceImpl implements IAnalysisService {
	
	private SimpleDateFormat dateFormat = new SimpleDateFormat("yyyy-MM-dd hh:mm:ss");
	
	@Autowired
	private SnProjectMapper snProjectMapper;
	
	@Autowired
	private SnItemMapper snItemMapper;
	
	@Autowired
	private TdreamSnProjectMapper tdreamSnProjectMapper;
	...
	@Autowired
	private TdreamSnAnalysisMapper tdreamSnAnalysisMapper;
	
	/**
	 * 如果在分析的过程中，出现异常，需要重新分析，会不会对已经分析的数据造成影响？
	 * 		事务回滚
	 * 		解决办法：删除当日的平台分析表（t_dream_Sn_analysis）和增速表（t_dream_rank_growth）的数据
	 * 	
	 * 	需要一个分析状态标识：如果已经完成分析了之后就不在进行分析
	 */
	@SuppressWarnings("unused")
	@Override
	public int doWebsiteAnalysis(Date updateDate,Logger logger) {
		logger.info("苏宁平台开始分析");
		//1、平台需要被分析的的产品
		long start = System.currentTimeMillis();
		List<TdreamSnProject> listByUpdateDate = snProjectMapper.selectListByUpdateDate(updateDate);
		long end = System.currentTimeMillis();
		logger.info("查询平台需要被分析的的产品花费"+(end-start)/1000+"秒");
		Map<String,Object> parmMap = new HashMap<String, Object>();
		Integer productId = null;
		for (TdreamSnProject tdreamSnProject : listByUpdateDate) {
			String originalId = tdreamSnProject.getOriginalId();
			//2、据平台编号和原始id查询t_dream_product表中
			TdreamProduct tdreamProduct = tdreamProductMapper.selectByOriginalId(7, originalId);
			if(tdreamProduct==null){
				tdreamSnProject.setBeginDate(new Date());
				//不存在，插入的产品到t_dream_product表
				tdreamSnProjectMapper.insertProduct(tdreamSnProject);
				productId = tdreamProductMapper.selectProductId(7,originalId);//改
			}else{
				productId = tdreamProduct.getPkId();
				tdreamSnProject.setBeginDate(tdreamProduct.getProductBegin());
				//存在，更新的产品到t_dream_product表
				tdreamSnProjectMapper.updateProduct(tdreamSnProject);
			}
			/**
			 * 处理当天重新分析，以及多次分析的情况
			 * 	根据更新时间删除当日的平台分析表（t_dream_Sn_analysis）和增速表（t_dream_rank_growth）的数据
			 */
			tdreamSnAnalysisMapper.deleteByProductId(productId,updateDate);
			 ...
			 * 先处理已经分类的产品，再进行分词分类后
			 */
			TdreamSnAnalysis entity = creatPrepareEntity(tdreamSnProject);
			entity.setProductId(productId);
			
			tdreamSnAnalysisMapper.insert(entity);
			tdreamSnAnalysisMapper.updateProduct(entity);
			
			if(tdreamProduct!=null&&tdreamProduct.getProductEnabled()==1){
				TdreamSnAnalysis prevEntity = tdreamSnAnalysisMapper.
						selectPrevAnalysis(productId,tdreamSnProject.getUpdateDate());//传入产品抓取回来传入数据库的时间：否则t_dream_rank_growth表的上一次金额为空
				insertRankGrowth(tdreamSnProject,entity,prevEntity,logger);
			}
		}
		long end2 = System.currentTimeMillis();
		logger.info("苏宁平台分析花费"+(end2-start)/1000+"秒");
		logger.info("苏宁平台分析结束");
		return 1;
	}
	
	
	
	private void insertRankGrowth(TdreamSnProject project, TdreamSnAnalysis entity, TdreamSnAnalysis prevEntity, Logger logger) {
		int days = 1;
	...
	private  TdreamSnAnalysis creatPrepareEntity(TdreamSnProject project){
		TdreamSnAnalysis entity = new TdreamSnAnalysis();
...
//		} else{
//			entity.setLikeCount(project.getLikeCount());
//		}
		...
		if(project.getItemList()!=null&&project.getItemList().size()>0){
			List<TdreamSnItem> limitList = new ArrayList<TdreamSnItem>();//支持人数无限额的支持档
			List<TdreamSnItem> unLimitList = new ArrayList<TdreamSnItem>();//支持人数有限额的支持档
			for(TdreamSnItem item : project.getItemList()){
			...
			if(limitList.size()>0){
				for(TdreamSnItem item : limitList){
				...
				if(unLimitList.size()>0){
					for(TdreamSnItem item : unLimitList){
						if(item.getItemPrice().intValue()==1){//如果支持档的价格是1，跳过执行下一次循环
							continue;
						}
						if(itemMap.get(item.getItemPrice())!=null){
	...
	public int doProductAnalysis(Integer originId) {
		List<TdreamSnProject> projectList = null; //产品列表
		List<TdreamSnItem> itemsList = null;	  //产品项列表
		...
		//获取指定的产品列表
		projectList = snProjectMapper.getProductByOriginIdWithDaySpace(originId);
		...
		for (TdreamSnProject tdreamSnProject : projectList) {
			//获取要展示产品的数据
			productName = tdreamSnProject.getProjectName();
			supportPersonList.add(String.valueOf(tdreamSnProject.getSupportPerson()));
			finishPercentList.add(String.valueOf(tdreamSnProject.getFinishPer()));
			currentMoneyList.add(String.valueOf(tdreamSnProject.getCurrMoney()));
			updateDateList.add(dateFormat.format(tdreamSnProject.getUpdateDate()));
			//获取这个产品下的项目
			itemsList = snItemMapper.getItemsByProjectId(tdreamSnProject.getPkId());
			if(itemsList==null||itemsList.size()==0){
				continue;
			}
			itemnum = itemsList.size();
			for (TdreamSnItem tdreamSnIterm : itemsList) {
				
				BigDecimal 	price = (BigDecimal) tdreamSnIterm.getItemPrice();
				Integer target = (Integer) tdreamSnIterm.getItemTotal();
				Integer actual = (Integer) tdreamSnIterm.getItemSupport();
