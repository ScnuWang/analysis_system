...
import cn.geekview.analysisSystem.entity.dao.TdreamXmAnalysisMapper;
import cn.geekview.analysisSystem.entity.dao.TdreamXmProjectMapper;
...
import cn.geekview.analysisSystem.remotedata.entity.dao.XmItemMapper;
import cn.geekview.analysisSystem.remotedata.entity.dao.XmProjectMapper;
...
@Service("XmAnalysisServiceImpl")
public class XmAnalysisServiceImpl implements IAnalysisService {
	...
	@Autowired
	private XmItemMapper xmItemMapper;
	
	@Autowired
	private TdreamXmProjectMapper tdreamXmProjectMapper;
	...
	@Autowired
	private TdreamXmAnalysisMapper tdreamXmAnalysisMapper;
	
	/**
	 * 如果在分析的过程中，出现异常，需要重新分析，会不会对已经分析的数据造成影响？
	 * 		事务回滚
	 * 		解决办法：删除当日的平台分析表（t_dream_Xm_analysis）和增速表（t_dream_rank_growth）的数据
	 * 	
	...
	public int doWebsiteAnalysis(Date updateDate,Logger logger) {
		logger.info("小米平台开始分析");
		//1、平台需要被分析的的产品
		long start = System.currentTimeMillis();
		List<TdreamXmProject> listByUpdateDate = xmProjectMapper.selectListByUpdateDate(updateDate);
		long end = System.currentTimeMillis();
		logger.info("查询平台需要被分析的的产品花费"+(end-start)/1000+"秒");
		Map<String,Object> parmMap = new HashMap<String, Object>();
		Integer productId = null;
		for (TdreamXmProject tdreamXmProject : listByUpdateDate) {
			String originalId = tdreamXmProject.getOriginalId();
			//2、据平台编号和原始id查询t_dream_product表中
			TdreamProduct tdreamProduct = tdreamProductMapper.selectByOriginalId(15, originalId);//修改平台编号
			if(tdreamProduct==null){
				tdreamXmProject.setBeginDate(new Date());
				//不存在，插入的产品到t_dream_product表
				tdreamXmProjectMapper.insertProduct(tdreamXmProject);
				productId = tdreamProductMapper.selectProductId(15,originalId);//修改平台编号
			}else{
				productId = tdreamProduct.getPkId();
				tdreamXmProject.setBeginDate(tdreamProduct.getProductBegin());
				//存在，更新的产品到t_dream_product表
				tdreamXmProjectMapper.updateProduct(tdreamXmProject);
			}
			/**
			 * 处理当天重新分析，以及多次分析的情况
			 * 	根据更新时间删除当日的平台分析表（t_dream_Xm_analysis）和增速表（t_dream_rank_growth）的数据
			 */
			tdreamXmAnalysisMapper.deleteByProductId(productId,updateDate);
			 ...
			 * 先处理已经分类的产品，再进行分词分类后
			 */
			TdreamXmAnalysis entity = creatPrepareEntity(tdreamXmProject);
			entity.setProductId(productId);
			
			tdreamXmAnalysisMapper.insert(entity);
			tdreamXmAnalysisMapper.updateProduct(entity);
			
			if(tdreamProduct!=null&&tdreamProduct.getProductEnabled()==1){
				TdreamXmAnalysis prevEntity = tdreamXmAnalysisMapper.
						selectPrevAnalysis(productId,tdreamXmProject.getUpdateDate());//传入产品抓取回来传入数据库的时间：否则t_dream_rank_growth表的上一次金额为空
				insertRankGrowth(tdreamXmProject,entity,prevEntity,logger);
			}
		}
		long end2 = System.currentTimeMillis();
		logger.info("小米平台分析花费"+(end2-start)/1000+"秒");
		logger.info("小米平台分析结束");
		return 1;
	}
	
	
	
	private void insertRankGrowth(TdreamXmProject project, TdreamXmAnalysis entity, TdreamXmAnalysis prevEntity, Logger logger) {
		int days = 1;
	...
	private  TdreamXmAnalysis creatPrepareEntity(TdreamXmProject project){
		TdreamXmAnalysis entity = new TdreamXmAnalysis();
...
//		}
		...
		if(project.getItemList()!=null&&project.getItemList().size()>0){
			List<TdreamXmItem> limitList = new ArrayList<TdreamXmItem>();//支持人数无限额的支持档
			List<TdreamXmItem> unLimitList = new ArrayList<TdreamXmItem>();//支持人数有限额的支持档
			for(TdreamXmItem item : project.getItemList()){
			...
			if(limitList.size()>0){
				for(TdreamXmItem item : limitList){
				...
				if(unLimitList.size()>0){
					for(TdreamXmItem item : unLimitList){
						if(item.getItemPrice().intValue()==1){//如果支持档的价格是1，跳过执行下一次循环
							continue;
						}
						if(itemMap.get(item.getItemPrice())!=null){
	...
	public int doProductAnalysis(Integer originId) {
		List<TdreamXmProject> projectList = null; //产品列表
		List<TdreamXmItem> itemsList = null;	  //产品项列表
		...
		//获取指定的产品列表
		projectList = xmProjectMapper.getProductByOriginIdWithDaySpace(originId);
		...
		for (TdreamXmProject tdreamXmProject : projectList) {
			//获取要展示产品的数据
			productName = tdreamXmProject.getProjectName();
			supportPersonList.add(String.valueOf(tdreamXmProject.getSupportPerson()));
			finishPercentList.add(String.valueOf(tdreamXmProject.getFinishPer()));
			currentMoneyList.add(String.valueOf(tdreamXmProject.getCurrMoney()));
			updateDateList.add(dateFormat.format(tdreamXmProject.getUpdateDate()));
			//获取这个产品下的项目
			itemsList = xmItemMapper.getItemsByProjectId(tdreamXmProject.getPkId());
			if(itemsList==null||itemsList.size()==0){
				continue;
			}
			itemnum = itemsList.size();
			for (TdreamXmItem tdreamXmIterm : itemsList) {
				
				BigDecimal 	price = (BigDecimal) tdreamXmIterm.getItemPrice();
				Integer target = (Integer) tdreamXmIterm.getItemTotal();
				Integer actual = (Integer) tdreamXmIterm.getItemSupport();
